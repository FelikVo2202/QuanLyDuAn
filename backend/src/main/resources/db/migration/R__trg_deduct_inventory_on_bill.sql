...
BEGIN
    ...
    bd.ProductID IS NOT NULL AND
    bd.QUANTITY > 0; -- ADDED: Only deduct if quantity is greater than 0
    ...
    WHERE bd.BillID = :NEW.BillID AND bd.SERVICEID IS NOT NULL AND bd.QUANTITY > 0; -- ADDED: Only deduct if quantity is greater than 0
